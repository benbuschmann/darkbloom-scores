...
- 31-day score retention and basic visitor/watch-time statistics.
...
| `PUBLIC_ORIGIN` | empty | Exact public origin, e.g. `https://scores.example.com`, no path |
| `TRUSTED_PROXY_CIDRS` | empty | Exact trusted proxy network(s), comma-separated |
...
- `GET /api/traffic` — aggregate traffic only, no raw IPs.
- `POST /api/view` and `/api/heartbeat` — same-origin JSON with a `session_id`.
...
Heartbeats run only while the page is visible. Sessions/IPs are approximate:
one IP can represent many people; multiple tabs can count separately. These
counters are not billing-grade or bot-proof.

Application traffic records, including IPs, are pruned after seven days.
Raw addresses are accessible only in the private database or local
`python3 server.py traffic-report` command. HTTP returns aggregates only.
Application request logging is disabled; proxy/CDN logs and backups have
independent retention. Backups can retain deleted records.

Forwarded IP headers are ignored unless the immediate peer is explicitly
trusted. Without proxy configuration, unique-IP counts behind Traefik will
be inaccurate. Follow the deployment guide before enabling that trust.

The per-IP event cap is a basic guardrail, not comprehensive abuse prevention.
Run this standard-library HTTP service behind Traefik with reasonable
request/rate limits. It is intended for modest traffic, not an unlimited API.
